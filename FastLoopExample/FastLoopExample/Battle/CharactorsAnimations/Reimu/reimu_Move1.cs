...
namespace FastLoopExample.Battle.CharactorsAnimations
{
    ...
    /// 蕾米利亚的一般站立动画
    /// </summary>
    public class reimu_stand : BattleAnimation
    {
        public reimu_stand(TextureManager texturemanager)
        {
            //站立动画一般都为循环式的
            needloop = true;
            //实例化动画数组
    ...
    /// 蕾米利亚的一般站立动画对应的适配器
    /// </summary>
    public class reimu_stand_Adaptor : Players.AnimationAdaptor
    {
        float width = 48;
        float height = 96;

        public reimu_stand_Adaptor(TextureManager texturemanager)
        {
            animation = new reimu_stand(texturemanager);
            collider = new RectCollider(48, 96);
        }
    ...
    public class reimu_walkFront : BattleAnimation
    {
        public reimu_walkFront(TextureManager texturemanager)
        {
            changeinterval = 0.08;
            //站立动画一般都为循环式的
            needloop = true;
    ...
    public class reimu_walkFront_Adaptor : Players.AnimationAdaptor
    {
        float width = 48;
        float height = 96;

        public reimu_walkFront_Adaptor(TextureManager texturemanager)
        {
             collider = new RectCollider(48, 96);
            animation = new reimu_walkFront(texturemanager);
        }
        public override void Start()
        {
            //调用Start以后，列表重置
    ...
    public class reimu_jump : BattleAnimation
    {
        public reimu_jump(TextureManager texturemanager)
        {
            needloop = false;
            changeinterval = 0.05;
            //站立动画一般都为循环式的
    ...
    public class reimu_jump_Adaptor : Players.AnimationAdaptor
    {
        float width = 48;
        float height = 96;

        public reimu_jump_Adaptor(TextureManager texturemanager)
        {
            animation = new reimu_jump(texturemanager);
